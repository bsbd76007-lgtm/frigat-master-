...
.infobar__nav { display: flex; align-items: center; gap: 4px; flex-wrap: wrap;
  margin-inline: auto; }
.infobar__link { padding: 6px 6px; font-size: 13px; font-weight: 600;
  color: var(--fg-muted); border-radius: var(--fg-r);
  transition: background-color var(--fg-t), color var(--fg-t); }
.infobar__link:hover { color: var(--fg-text); background: var(--fg-hover); }
.infobar__link--on { color: var(--fg-text); background: var(--fg-hover); }
.infobar__cta { flex: none; padding: 8px 10px; font-size: 12.5px; font-weight: 800;
  color: var(--fg-bg); background: var(--fg-accent); border-radius: var(--fg-r);
  transition: filter var(--fg-t), transform var(--fg-t); }
.infobar__cta:hover { filter: brightness(1.06); transform: translateY(-1px); }

.info { width: 100%; max-width: 940px; margin-inline: auto; padding: 16px 12px 56px;
...
.info__list { margin: 0 0 10px; padding-left: 16px; }
...
.info__card { display: flex; flex-direction: column; gap: 8px; padding: 12px;
  background: var(--fg-panel); border: 1px solid transparent; border-radius: var(--fg-r-lg); }
...
.info__tag { align-self: flex-start; padding: 2px 8px; font-size: 10.5px; font-weight: 800;
  letter-spacing: .06em; text-transform: uppercase; border-radius: var(--fg-r);
